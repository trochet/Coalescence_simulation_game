...
#Number of possible lineage sites per population (will be assigned letters of the alphabet)
n_lin<-13
...
#Can also set sites to be contiguous

#mut_sites_pop1<-sites_pop1[1:n_mut_sites]
#mut_sites_pop2<-sites_pop2[1:n_mut_sites]
#mut_sites<-union(mut_sites_pop1,mut_sites_pop2)

#recomb_sites_pop1<-sites_pop1[(n_mut_sites+2):(n_recomb_sites+n_mut_sites+1)]
#recomb_sites_pop2<-sites_pop2[(n_mut_sites+2):(n_recomb_sites+n_mut_sites+1)]
#recomb_sites<-union(recomb_sites_pop1,mut_sites_pop2)

#mut_recomb_sites_pop1<-sites_pop1[n_mut_sites+1]
#mut_recomb_sites_pop2<-sites_pop1[n_mut_sites+1]
#mut_recomb_sites<-union(recomb_sites_pop1,mut_sites_pop2)
...
while(max(count_players(rounds[nrow(rounds),]))<20){
    #Mutations
    ...
    for(s in all_recomb_sites){
        if(is.na(rounds[nrow(rounds)-1,which(names(rounds)==s)])){
            ...
            if(s %in% sites_pop1){
                if(p_dest %in% sites_pop2){
                    p_dest<-sample(sites,1,prob=rep(1/(2*n_lin),2*n_lin))
                }
                if(q_dest %in% sites_pop2){
                    q_dest<-sample(sites,1,prob=rep(1/(2*n_lin),2*n_lin))
                }
            } else {
                if(p_dest %in% sites_pop1){
                    p_dest<-sample(sites,1,prob=rep(1/(2*n_lin),2*n_lin))
                }
                if(q_dest %in% sites_pop1){
                    q_dest<-sample(sites,1,prob=rep(1/(2*n_lin),2*n_lin))
                }
            }
    ...
    for(s in non_recomb_sites){
        if(is.na(rounds[nrow(rounds)-1,which(names(rounds)==s)])){
            ...
            if(s %in% sites_pop1){
                if(dest %in% sites_pop2){
                    dest<-sample(sites,1,prob=rep(1/(2*n_lin),(2*n_lin)))
                }
            } else {
                if(dest %in% sites_pop1){
                    dest<-sample(sites,1,prob=rep(1/(2*n_lin),(2*n_lin)))
                }
            }
            site_players<-unlist(strsplit(rounds[nrow(rounds)-1,which(names(rounds)==s)],","))
